// supabase/functions/get-exams/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("exam_titles").select("title, default_max_score").eq("teacher_id", finalClientId).order("title");
    ...
    return new Response(JSON.stringify({ success: true, data: (data || []).map((r: any) => ({ title: r.title, defaultMaxScore: r.default_max_score })) }),
