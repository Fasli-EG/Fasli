// supabase/functions/check-student-uid/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const { clientId, uid } = await req.json();

    if (!clientId || !uid) {
      return new Response(JSON.stringify({ success: false, message: "بيانات ناقصة" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    const finalClientId = requireOwnClientId(payload, clientId);

    const { data: student, error } = await supabase
      .from("students").select("name, uid").eq("teacher_id", finalClientId).eq("uid", uid).maybeSingle();
    ...
    if (student) {
      return new Response(JSON.stringify({ success: true, exists: true, student: { name: student.name, uid: student.uid } }),
