...
async function licenseCheckClient() {
  ...
  // ✅ Aug 2026 (Phase I follow-up 10 fix): كانت ترتيب المتغيرات هنا معكوسة عن باقي كل دوال
  // المشروع (DATABASE_URL/SERVICE_ROLE الأول بدل SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY) —
  // ده كان بيخلّي فحص الترخيص في الدالة دي بالذات (وبس) يتصل بعنوان غلط لو DATABASE_URL متضبط
  // لحاجة تانية (زي سلسلة اتصال Postgres مباشرة)، فيفشل الاستعلام ويرجّع "الحساب غير موجود"
  // دايماً — بينما كل الدوال التانية شغّالة صح لأنها بتفضّل SUPABASE_URL. اتصلح ليطابق الباقي.
